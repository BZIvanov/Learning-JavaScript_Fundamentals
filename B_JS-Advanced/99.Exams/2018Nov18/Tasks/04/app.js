  ...
  findOfferButton.on('click', function () {
    ...
    if (secondAreValid) {
      ...
      for (let i = 0; i < availableOffers.length; i++) {
        if (
          searchedType.val() === availableOffers[i].apartType &&
          +familyBudget.val() >= availableOffers[i].total
        ) {
          entriesValid = false;
          ...
          pbutton.on('click', function () {
            notification.empty();
            let noty = $(
              `<p id="message">They had found cockroaches in ${fam}\'s apartment</p>`
            );
            $(this).parent().remove();
          ...
          agencyProfit +=
            availableOffers[i].priceAsked *
            (availableOffers[i].comRate / 100) *
            2;
