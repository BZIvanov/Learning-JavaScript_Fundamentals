...
const SoftUniFy = require('../02.TestTask').SoftUniFy;
...
describe('Test Softunify class', function () {
  ...
  describe('test downloadSong method', function () {
    it('normal case', function () {
      ...
      expect(result.allSongs).to.have.deep.property('Eminem', {
        rate: 0,
        votes: 0,
        songs: ['Hello - How nice...'],
      });
    });
  });
  describe('test playSong method', function () {
    it('for no songs downloaded', function () {
      expect(testCase.playSong('Hello')).to.equal(
        "You have not downloaded a Hello song yet. Use SoftUniFy's function downloadSong() to change that!"
      );
    });
  ...
  describe('test songsList getter', function () {
    it('for some songs downloaded', function () {
      ...
      result = testCase.downloadSong('Riana', 'What', 'Now');
      result = testCase.downloadSong('Eminem', 'Why', 'How');
      expect(result.songsList).to.equal(
        'Hello - Wow...\nWhy - How\nWhat - Now'
      );
    });
    it('for some songs downloaded', function () {
  ...
  describe('test rateArtist method', function () {
    it('for some songs downloaded', function () {
      ...
      result = testCase.downloadSong('Riana', 'What', 'Now');
      result = testCase.downloadSong('Eminem', 'Why', 'How');
      expect(result.rateArtist('Jovi')).to.equal(
        'The Jovi is not on your artist list.'
      );
    });
  });
});
