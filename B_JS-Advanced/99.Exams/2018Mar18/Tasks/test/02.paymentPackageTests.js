...
const PaymentPackage = require('../02.paymentPackage');
...
describe('Test PaymentPackage class', function () {
  ...
  describe('test value property', function () {
    it('incorrect value', function () {
      expect(() => {
        new PaymentPackage('Consultation', 'hi');
      }).to.throw(Error);
      expect(() => {
        new PaymentPackage('Consultation', '150');
      }).to.throw(Error);
      expect(() => {
        new PaymentPackage('Consultation', -10);
      }).to.throw(Error);
      expect(() => {
        new PaymentPackage('Consultation', -1000.1);
      }).to.throw(Error);
    });
    it('with zero', function () {
      const pack = new PaymentPackage('Consultation', 0);
  ...
  describe('test VAT property', function () {
    it('incorrect value', function () {
      const pack = new PaymentPackage('Consultation', 100);
      expect(() => {
        pack.VAT = 'number';
      }).to.throw(Error);
      expect(() => {
  ...
  describe('test active property', function () {
    it('incorrect value', function () {
      const pack = new PaymentPackage('Partnership Fee', 1200);
      expect(() => {
        pack.active = 'number';
      }).to.throw(Error);
      expect(() => {
  ...
  describe('test toString method', function () {
    ...
    it('active property to false', function () {
      const pack = new PaymentPackage('Partnership Fee', 1000);
      pack.active = false;
      expect(pack.toString()).to.equal(
