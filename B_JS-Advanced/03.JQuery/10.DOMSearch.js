function domSearch(selector, isCaseSensitive) {
    ...
    $(container).append(
      $('<div>')
        ...
        .append(
          $('<label>')
            .text('Search:')
            .append($('<input>').on('input', searchWithQuery))
        )
    );
  }
  ...
  function addElementToList() {
    ...
    $('.result-controls .items-list').append(
      $('<li>')
        .addClass('list-item')
        .append($('<a>').addClass('button').text('X').on('click', deleteItem))
        .append($('<strong>').text(textValue))
    );
  }
  ...
  function searchWithQuery() {
    ...
    if (isCaseSensitive) {
      ...
      $('.list-item strong')
        ...
        .filter(
          (item) =>
            item.textContent.toLowerCase().indexOf(queryStr.toLowerCase()) ===
            -1
        )
        .forEach((item) => (item.parentNode.style.display = 'none'));
    }
