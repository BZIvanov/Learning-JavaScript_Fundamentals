const manager = (function () {
  ...
  function prepare([recipe, quantity]) {
    ...
    const totalRequredProteins = recipes.get(recipe).protein * quantity;
    const totalRequredCarbohydrates =
      recipes.get(recipe).carbohydrate * quantity;
    const totalRequredFats = recipes.get(recipe).fat * quantity;
    const totalRequredFlavours = recipes.get(recipe).flavour * quantity;

    if (totalRequredProteins > store.protein)
      return 'Error: not enough protein in stock';
    if (totalRequredCarbohydrates > store.carbohydrate)
      return 'Error: not enough carbohydrate in stock';
    if (totalRequredFats > store.fat) return 'Error: not enough fat in stock';
    if (totalRequredFlavours > store.flavour)
  ...
  return function (inputLine) {
    ...
    switch (command) {
      case 'restock':
        return restock(cmdArgs);
      case 'prepare':
        return prepare(cmdArgs);
      case 'report':
        return report();
    }
  };
