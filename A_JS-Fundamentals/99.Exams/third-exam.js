...
function arenaTier(arr) {
  ...
  for (let row of arr) {
    ...
    if (type > -1 && row !== 'Ave Cesar') {
      ...
      if (gladiators.hasOwnProperty(g1) && gladiators.hasOwnProperty(g2)) {
        let g1Skills = Object.keys(gladiators[g1]).filter(
          (x) => x !== '__total__'
        );
        let g2Skills = Object.keys(gladiators[g2]).filter(
          (x) => x !== '__total__'
        );
  ...
  for (let key of sortedKeys) {
    let nestSorted = Object.keys(gladiators[key])
      .filter((x) => x !== '__total__')
      .sort((a, b) => {
