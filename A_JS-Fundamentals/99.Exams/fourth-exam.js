...
function travelPlans(arr) {
  ...
  if (goldObtained < 1000) {
    ...
    console.log(
      `Mariyka need to earn ${(1000 - goldObtained).toFixed(
        2
    ...
    console.log(`Final sum: ${goldObtained.toFixed(2)}`);
    console.log(
      `Mariyka earned ${(goldObtained - 1000).toFixed(2)} gold more.`
    );
  }
}
travelPlans([
...
function minkeDecode(arr) {
  ...
  let realCountry =
    country.substring(0, start) +
    piece +
    country.substring(end + 1, country.length - 1) +
    country[country.length - 1].toLowerCase();

  let town = arr[0]
    .match(regexCity)
    .map((x) => String.fromCharCode(Math.ceil(+x)))
    .map((el, index) => {
      if (index === 0) {
