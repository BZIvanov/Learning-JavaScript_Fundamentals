function renderHomeView(flights) {
  for (const flight of flights) {
    const a = $(`<a href="#" class="added-flight"></a>`).on(
      'click',
      function () {
    ...
    a.append(
      $(`<img src="${flight.img}" alt="" class="picture-added-flight">`)
    );
    a.append($(`<h3>${flight.destination}</h3>`));
    a.append(
      $(
        `<span>from ${flight.origin}</span><span>${flight.departureDate}</span>`
      )
    );
...
function renderDetailsView(flight) {
  ...
  const mainDiv = $(`<div class="ticket-area"></div>`);
  mainDiv.append(
    $(`<div class="ticket-area-left"><img src="${flight.img}" alt=""></div>`)
  );
  const innerDiv = $(`<div class="ticket-area-right"></div>`);
  innerDiv.append(
    $(`<h3>${flight.destination}</h3><div>from ${flight.origin}</div>`)
  );
  let mostInnerDiv = $(
    `<div class="data-and-time">${flight.departureDate} ${flight.departureTime}</div>`
  );
  if (sessionStorage.getItem('userId') === flight._acl.creator) {
    mostInnerDiv.append(
      $(`<a href="#" class="edit-flight-detail"></a>`).on('click', function (
        e
      ) {
        e.stopPropagation();
  ...
  innerDiv.append(
    $(`<div>${flight.seats} Seats (${flight.cost} per seat)</div>`)
  );
...
function renderMyFlights(flights) {
  ...
  for (const f of flights) {
    const mainDiv = $(`<div class="flight-ticket"></div>`);
    mainDiv.append(
      $(`<div class="flight-left"><img src="${f.img}" alt=""></div>`)
    );
    ...
    innerDiv.append(
      $(`<a href="#" class="remove">REMOVE</a>`).on('click', function (event) {
        kinveyRequester.removeFlight(f._id);
      })
    );
    innerDiv.append(
      $(`<a href="#" class="details">Details</a>`).on('click', function (
        event
      ) {
        renderDetailsView(f);
      })
