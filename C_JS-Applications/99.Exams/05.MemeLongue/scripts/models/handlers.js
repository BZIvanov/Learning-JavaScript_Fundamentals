handlers.displayHomePage = function (context) {
  if (context.isLogged) {
    requester
      .get('appdata', 'memes?query={}&sort={"_kmd.ect":-1}', 'kinvey')
...
handlers.registerNewUser = function (context) {
  ...
  if (!usernameRegex.test(username)) {
    ...
    servicer.showError(
      'Password should contain 6 or more latin letters or digits!'
    );
...
handlers.uploadNewMeme = function (context) {
  ...
  if (title.length === 0 || title.length > 33) {
    ...
    requester
      .post('appdata', 'memes', 'kinvey', data)
      .then(function (response) {
...
handlers.deleteMeme = function (context) {
  ...
  requester
    .remove('appdata', `memes/${memeId}`, 'kinvey')
    .then(function () {
